...
Additional AORC package dependencies:

- No AORC-specific packages.
- Inherits the baseline runtime additions:
  `torchnet==0.0.4`, `visdom==0.2.4`, `tornado==6.5.7`,
  `jsonpatch==1.33`, `websocket-client==1.9.0`.

AORC uses PyTorch modules only. The age-label loader accepts text mappings
or NumPy `.npy` dictionaries through dependencies already used by WeSpeaker.
