        ...
        for tarinfo in stream:
            ...
            with stream.extractfile(tarinfo) as file_obj:
                try:
                    ...
                    elif postfix in ['age_group', 'age']:
                        example['age_group'] = int(file_obj.read().decode('utf8').strip())
                    else:
...
def parse_raw(data):
    ...
    for sample in data:
        ...
        try:
            ...
            if 'age_group' in obj:
                example['age_group'] = int(obj['age_group'])
            elif 'age' in obj:
                example['age_group'] = int(obj['age'])
            yield example
        except Exception as ex:
...
def filter_keys(data, key_set):
    """Keep only samples whose utterance key is in key_set."""
    for sample in data:
        assert 'key' in sample
        if sample['key'] in key_set:
            yield sample
...
def spk_to_id(data, spk2id):
    ...
    for sample in data:
        ...
        if sample['spk'] in spk2id:
            ...
            label = -1
        sample['label'] = label
        if 'age' in sample and 'age_group' not in sample:
            sample['age_group'] = int(sample['age'])
        yield sample


def age_to_group(data, age_labels, ignore_index=-1):
    """Attach optional age group labels by utterance key."""
    for sample in data:
        assert 'key' in sample
        key = sample['key']
        candidates = [key]
        ...
        for candidate in candidates:
            if candidate in age_labels:
                age_group = age_labels[candidate]
                break
...
def prepare_waveform_for_cvac(data,
                              waveform_sample_rate=16000,
                              waveform_max_seconds=3.0,
                              waveform_key='waveform',
                              normalize_waveform=True):
    """Attach an optional fixed-length waveform for Waveform-CVAC training."""
    target_len = int(waveform_sample_rate * waveform_max_seconds)
    for sample in data:
        ...
        if wav.numel() > target_len:
            ...
            wav = wav[start:start + target_len].clone()
        elif wav.numel() < target_len:
            wav = F.pad(wav, (0, target_len - wav.numel()))
        sample[waveform_key] = wav.clamp(-1.0, 1.0)
        yield sample
...
def compute_fbank(data,
    ...
    for sample in data:
        ...
        if 'age_group' in sample:
            out['age_group'] = int(sample['age_group'])
        if return_waveform and waveform_key in sample:
            out[waveform_key] = sample[waveform_key]
        yield out
