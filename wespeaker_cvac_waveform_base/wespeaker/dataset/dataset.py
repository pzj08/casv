...
def Dataset(data_type,
    ...
    """ Construct dataset from arguments
        ...
        Args:
            ...
            key_filter_file: optional one-column file of utterance keys to keep
    """
    ...
    key_filter = key_filter_file or configs.get('key_filter_file', None)
    if key_filter:
        key_set = set(read_lists(key_filter))
        dataset = Processor(dataset, processor.filter_keys, key_set)
    ...
    if age_labels is not None:
        dataset = Processor(dataset, processor.age_to_group, age_labels,
                            ignore_age_index)
    ...
    if data_type == 'feat':
        ...
        if (reverb_lmdb_file and noise_lmdb_file) and (aug_prob > 0.0):
            ...
            dataset = Processor(dataset, processor.add_reverb_noise,
                                ...
                                aug_prob)
        if configs.get('return_waveform', False):
            dataset = Processor(
                dataset, processor.prepare_waveform_for_cvac,
                waveform_sample_rate=configs.get('waveform_sample_rate',
                                                 resample_rate),
                waveform_max_seconds=configs.get('waveform_max_seconds', 3.0),
                waveform_key=configs.get('waveform_key', 'waveform'),
                normalize_waveform=configs.get('normalize_waveform', True))
        # compute fbank
        if frontend_type == 'fbank':
            dataset = Processor(
                dataset, processor.compute_fbank,
                return_waveform=configs.get('return_waveform', False),
                waveform_key=configs.get('waveform_key', 'waveform'),
                **configs['fbank_args'])
